...
function f_1_1 { # Initial VM Setup Tasks
	...
	f_2_11 # 2.11 - Set up SSH keys
	f_2_14 # 2.14 - Sudo Insults
	f_2_15 # 2.15 - Install QEMU Guest Agent"
	f_2_16 # 2.16 - Add bash aliases
	f_1_2  # Disk Cleanup and Setting Optomization
	sudo cp /usr/share/zoneinfo/America/New_York /etc/localtime # set timezone, make into its own option when im not lazy
}
...
function f_2_5 { # 2.5 - Install Tools (and fix vnstat config)
	sudo apt-get update && sudo apt-get install cifs-utils htop iotop vnstat tmux ncdu dfc rsync unzip zip openssh-server make git vim curl jq cowsay fortune sl cmatrix -y
...
function f_2_9 { # 2.9 - Install/Update Docker and Compose
	...
	curl -L https://github.com/docker/compose/releases/download/1.20.1/docker-compose-`uname -s`-`uname -m` | sudo tee -a /usr/local/bin/docker-compose > /dev/null 
